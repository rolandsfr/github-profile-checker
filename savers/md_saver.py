    ...
    with open(f"data/summary_{profile_name}.md", "w") as file:
        ...
        for summary_type in merged_summary.keys():
            ...
            if summary_type == "plain":
                ...
                file.write("<p>All your personal repositories (non-forked <b>and</b> not a part of any organization)</p>\n\n")
            elif summary_type == "with_orgs":
                file.write("## Organization repositories\n")
                file.write("<p>All the repositories that are part of your organizations</p>\n\n")
            elif summary_type == "profile":
                file.write("## Profile analysis\n")
                file.write("<p>Important pieces that might be missing in your public developer GitHub profile</p>\n\n")

            target_index = find_index(lambda target_type: summary_type == target_type["name"], head_columns)

            if summary_type != "profile":
                headers = list(map(lambda col: col["name"], head_columns[target_index]["columns"]))
                ...
                for entry in merged_summary[summary_type]:
                    ...
                    for criteria in head_columns[target_index]["columns"]:
                        ...
                        if unit == "is_empty":
                            ...
                            value = not value["camelcase"] and not value["other_unsupported"]
                        elif unit == "similar":
                            value = "<mark>" + ", ".join(value) + "</mark>" if len(value) else str(None)
                ...
                for entry in merged_summary[summary_type].keys():
                    value_matrix.append([entry, "Yes" if merged_summary[summary_type][entry] else "<mark>No</mark>"])

            writer = MarkdownTableWriter(headers=headers, value_matrix=value_matrix, )
